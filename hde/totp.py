...
def custom(email):
    secret = ("%s%s" % (sys.argv[1], "HDECHALLENGE003")).encode("utf-8")
    tmp = "%s:%s" % (email, str(totp(secret)).rjust(DIGITS, "0"))
    return base64.encodestring(tmp.encode("utf-8")).decode("utf-8").strip()
